...
class RaftServer(val id: Int, val servidores: List<Cliente>) :
    ...
    init {
        ...
        kLogger.info { "Servidor $id é $estado na rodada $termoAtual" }
    ...
    //fim das config de visualização de propagação



    //inicio de configurações de eleição
                ...
                launch {
                    try {
                        ...
                        println("Anexo id: ${it.id} termo atual: ${termoAtual} " +
                                    "Index do log anterior ${indexLogAnterior} Log anterior ${termoLogAnterior} entradas ${entradas} indice do envio ${commitIndex}")
    ...
    private suspend fun eleicaoLider() {
        ...
        while (estado == Estados.CANDIDATO) {
            ...
            kLogger.info { "Servidor $id está como $estado na rodada $termoAtual e votos = ${votosConcedidos.get()}" }
        }
    ...
    override suspend fun append(request: RequestAppendEntriesRPC):
        ...
        kLogger.info { "Lider eleito: ${request.leaderId} na rodada: ${request.term}" }
        ...
        kLogger.info { "Estado: ${termoAtual} ordem De Envio: $commitIndex " }
...
//configuração de instanciação
//passo minha lista de clientes como parametro
private fun raftInstance(id: Int, porta: Int, servidores: List<Cliente>) {
    val log = KotlinLogging.logger("servidor")

    log.info { "Iniciando servidor $id na porta $porta" }
...
fun main(args: Array<String>) {
    ...
    raftInstance(id = 2, porta = 8082, servidores = servidores)
}